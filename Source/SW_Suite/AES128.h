/*#ifndef AES128_H
#define AES128_H

#include <iostream> 
#include <sstream>
#include <vector>
#include <stdexcept>
#include <iomanip>
#include <fstream>
#include <cstdint>
#include "BaseConversions.h"
//#include <openssl/evp.h>


using namespace std;

vector<uint8_t> decryptHexString(const string& hexData, const string& key);
string bytesToHexString(const vector<uint8_t>& bytes);
vector<uint8_t> hexStringToBytes(const string& hexString);


// AES block size in bytes
const int BLOCK_SIZE = 16;

// S-box for AES
const uint8_t sBox[256] = {
    
};

// Inverse S-box for AES
const uint8_t invSBox[256] = {
    ,
};

// Round constants
const uint8_t rCon[10] = {
    0x01, 0x02, 0x04, 0x08, 0x10,
    0x20, 0x40, 0x80, 0x1B, 0x36
};

// Function declarations
void keyExpansion(const uint8_t* key, uint8_t* expandedKey);
void decryptBlock(uint8_t state[4][4], const uint8_t* expandedKey);
void invSubBytes(uint8_t state[4][4]);
void invShiftRows(uint8_t state[4][4]);
void addRoundKey(uint8_t state[4][4], const uint8_t* roundKey, unsigned char round = 0);
void invMixColumns(uint8_t state[4][4]);
void rotateRowRight(uint8_t state[4][4], short int row = 0, short int d = 0);
unsigned int RotWord(unsigned int _w);
unsigned int SubWord(unsigned int _w);
vector<uint8_t> decryptHexString(const string& hexData, const string& key);
vector<uint8_t> decryptHexFile(const string& hexFilePath, const string& key);
string bytesToHexString(const vector<uint8_t>& bytes);
vector<uint8_t> hexStringToBytes(const string& hexString);

// Padding functions
vector<uint8_t> addPadding(const vector<uint8_t>& input);
vector<uint8_t> removePadding(const vector<uint8_t>& input);

vector<uint8_t> hexStringToBytes(const string& hex) {
    vector<uint8_t> bytes;
    for (size_t i = 0; i < hex.length(); i += 2) {
        string byteString = hex.substr(i, 2);
        uint8_t byte = static_cast<uint8_t>(stoul(byteString, nullptr, 16));
        bytes.push_back(byte);
    }
    return bytes;
}

string bytesToHexString(const vector<uint8_t>& bytes) {
    stringstream ss;
    for (const auto& byte : bytes) {
        ss << hex << setw(2) << setfill('0') << static_cast<int>(byte);
    }
    return ss.str();
}

void decryptBlock(uint8_t state[4][4], const uint8_t* expandedKey) {
    const int Nr = 10; // Number of rounds for AES-128

    // AddRoundKey (the last round key is added first)
    addRoundKey(state, expandedKey, 10);

    for (int round = Nr - 1; round > 0; --round) {
        invShiftRows(state); // Inverse Shift Rows
        invSubBytes(state);   // Inverse Sub Bytes
        invMixColumns(state); // Inverse Mix Columns
        addRoundKey(state, expandedKey, round); // Add Round Key
    }

    // Final round (no InvMixColumns)
    invShiftRows(state);
    invSubBytes(state);
    addRoundKey(state, expandedKey); // Add initial round key
}

void addRoundKey(uint8_t state[4][4], const uint8_t* roundKey, unsigned char round) {
    for (int j = 0; j < 4; ++j) {
        for (int k = 0; k < 4; ++k) {
            state[j][k] ^= roundKey[(round * 16) + (j * 4) + k];
        }
    }
}

void invSubBytes(uint8_t state[4][4]) {
    // Apply the inverse S-box transformation to each byte in the state
    for (int j = 0; j < 4; ++j) {
        for (int k = 0; k < 4; ++k) {
            state[j][k] = invSBox[state[j][k]];
        }
    }
}

void invShiftRows(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++)
    {
        rotateRowRight(state, i, i);
    }
}

void rotateRowRight(uint8_t state[4][4], short int row, short int d)
{
    // Shift rows right by different offsets
    uint8_t temp;

    for (int i = 0; i < d; i++)
    {
        //row is shifted right by 1
        temp = state[3][row];

        state[3][row] = state[2][row];
        state[2][row] = state[1][row];
        state[1][row] = state[0][row];
        state[0][row] = temp;
    }
}


void keyExpansion(const uint8_t* key, uint8_t* expandedKey) {
    const int Nk = 4; // Number of 32-bit words in the key (AES-128 has a key size of 128 bits, or 4 words)
    const int Nr = 10; // Number of rounds for AES-128
    const int Nb = 4; // Number of 32-bit words in a block

    // Copy the original key into the beginning of expandedKey
    for (int i = 0; i < Nk; ++i) {
        expandedKey[i * 4] = key[i * 4];
        expandedKey[i * 4 + 1] = key[i * 4 + 1];
        expandedKey[i * 4 + 2] = key[i * 4 + 2];
        expandedKey[i * 4 + 3] = key[i * 4 + 3];
    }

    //most likely current culprit
    
    //unsigned int temp;
    uint8_t temp[4];
    for (int i = Nk; i < (Nb * (Nr + 1)); ++i) {
        // Get the previous word
        temp[0] = expandedKey[(i - 1) * 4];
        temp[1] = expandedKey[(i - 1) * 4 + 1];
        temp[2] = expandedKey[(i - 1) * 4 + 2];
        temp[3] = expandedKey[(i - 1) * 4 + 3];
        
        //Get the previous word
        //temp = (expandedKey[(i - 1) * 4] << 24) | (expandedKey[(i - 1) * 4 + 1] << 16) | (expandedKey[(i - 1) * 4 + 2] << 8) | expandedKey[(i - 1) * 4 + 3];

        if (i % Nk == 0) {
            
            // Perform a rotation and apply the S-box
            uint8_t t = temp[0];
            temp[0] = sBox[temp[1]];
            temp[1] = sBox[temp[2]];
            temp[2] = sBox[temp[3]];
            temp[3] = sBox[t];
            
            /*
            // Perform a rotation and apply the S-box
            uint8_t t = temp[3];
            temp[3] = sBox[temp[2]];
            temp[2] = sBox[temp[1]];
            temp[1] = sBox[temp[0]];
            temp[0] = sBox[t];
            *

            // XOR with the round constant
            temp[0] ^= rCon[(i / Nk) - 1];

            //temp = SubWord(RotWord(temp));
            //temp ^= rCon[i / Nk];
        }
        else if (Nk > 6 && i % Nk == 4) {
            // Apply S-box only
            for (int j = 0; j < 4; ++j) {
                temp[j] = sBox[temp[j]];
            }
        }

        
        // XOR with the word Nk positions back
        expandedKey[i * 4] = expandedKey[(i - Nk) * 4] ^ temp[0];
        expandedKey[i * 4 + 1] = expandedKey[(i - Nk) * 4 + 1] ^ temp[1];
        expandedKey[i * 4 + 2] = expandedKey[(i - Nk) * 4 + 2] ^ temp[2];
        expandedKey[i * 4 + 3] = expandedKey[(i - Nk) * 4 + 3] ^ temp[3];
    }
    
    //end most likely current culprit
}

void invMixColumns(uint8_t state[4][4]) {
    uint8_t temp[4][4];

    for (int j = 0; j < 4; j++) {
        temp[j][0] = (0x0e * state[j][0]) ^ (0x0b * state[j][1]) ^ (0x0d * state[j][2]) ^ (0x09 * state[j][3]);
        temp[j][1] = (0x09 * state[j][0]) ^ (0x0e * state[j][1]) ^ (0x0b * state[j][2]) ^ (0x0d * state[j][3]);
        temp[j][2] = (0x0d * state[j][0]) ^ (0x09 * state[j][1]) ^ (0x0e * state[j][2]) ^ (0x0b * state[j][3]);
        temp[j][3] = (0x0b * state[j][0]) ^ (0x0d * state[j][1]) ^ (0x09 * state[j][2]) ^ (0x0e * state[j][3]);
    }

    for (int j = 0; j < 4; j++)
        for (int k = 0; k < 4; k++)
            state[j][k] = temp[j][k];
}

vector<uint8_t> decryptHexFile(const string& hexFilePath, const string& key) {
    // Open the hex file
    ifstream infile(hexFilePath);
    if (!infile.is_open()) {
        throw runtime_error("Could not open the hex file.");
    }

    // Read the hex content
    string hexData((istreambuf_iterator<char>(infile)), istreambuf_iterator<char>());
    infile.close();

    return decryptHexString(hexData, key);
}

vector<uint8_t> decryptB64String(const string& B64Data, const string& key) {
    return decryptHexString(b64_str_to_hex_str(B64Data), key);
}

vector<uint8_t> decryptHexString(const string& hexData, const string& key) {
    // Convert hex string to bytes
    vector<uint8_t> encryptedData = hexStringToBytes(hexData);
    // Prepare expanded key
    uint8_t expandedKey[176]; // 16 bytes * (10 rounds + 1)
    keyExpansion(reinterpret_cast<const uint8_t*>(key.c_str()), expandedKey);
    // Prepare the decrypted data buffer
    vector<uint8_t> decryptedData(encryptedData.size());

    // Decrypt each 16-byte block
    uint8_t block[16];
    uint8_t state[4][4];
    for (size_t i = 0; i < encryptedData.size(); i += BLOCK_SIZE) {
        // Check for a full block
        if (i + BLOCK_SIZE > encryptedData.size()) {
            cerr << "Warning: Incomplete block at end of file." << endl;
            break;
        }

        // Copy the next 16 bytes to the block
        memcpy(block, &encryptedData[i], BLOCK_SIZE);

        // Convert the block to a 4x4 matrix
        for (int j = 0; j < 4; ++j)
            for (int k = 0; k < 4; ++k)
                state[j][k] = block[j * 4 + k];

        // Decrypt the block
        decryptBlock(state, expandedKey);

        // Convert the decrypted matrix back to bytes
        for (int j = 0; j < 4; ++j)
            for (int k = 0; k < 4; ++k)
                block[j * 4 + k] = state[j][k];

        // Store the decrypted block
        memcpy(&decryptedData[i], block, BLOCK_SIZE);
    }

    return decryptedData;
}

// Adds PKCS#7 padding to the input vector
vector<uint8_t> addPadding(const vector<uint8_t>& input) {
    size_t paddingSize = BLOCK_SIZE - (input.size() % BLOCK_SIZE);
    std::vector<uint8_t> paddedInput = input;

    // Add padding bytes
    for (size_t i = 0; i < paddingSize; ++i) {
        paddedInput.push_back(static_cast<uint8_t>(paddingSize));
    }

    return paddedInput;
}

// Removes PKCS#7 padding from the input vector
vector<uint8_t> removePadding(const vector<uint8_t>& input) {
    if (input.empty()) {
        throw std::invalid_argument("Input is empty");
    }

    // Get the last byte which indicates the number of padding bytes
    uint8_t paddingSize = input.back();

    // Check for valid padding size
    if (paddingSize < 1 || paddingSize > BLOCK_SIZE) {
        throw std::invalid_argument("Invalid padding size");
    }

    // Check if the padding bytes match the padding size
    for (size_t i = 1; i <= paddingSize; ++i) {
        if (input[input.size() - i] != paddingSize) {
            throw std::invalid_argument("Invalid padding");
        }
    }

    // Remove padding
    return vector<uint8_t>(input.begin(), input.end() - paddingSize);
}


#endif // AES128_H
*/
#pragma once

#include <iostream>
#include <vector>
#include <string>
#include <cstring>
#include <fstream>
#include <sstream>
#include <cstdint>
#include "BaseConversions.h"

// AES block size in bytes
const int BLOCK_SIZE = 16;

// AES S-box
const unsigned char sbox[256] = {
     
};

// AES inverse S-box
const unsigned char inv_sbox[256] = {
     
};

// AES round constant
const unsigned char Rcon[11] = {
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

// Adds PKCS#7 padding to the input vector
vector<uint8_t> addPadding(const vector<uint8_t>& input) {
    size_t paddingSize = BLOCK_SIZE - (input.size() % BLOCK_SIZE);
    std::vector<uint8_t> paddedInput = input;

    // Add padding bytes
    for (size_t i = 0; i < paddingSize; ++i) {
        paddedInput.push_back(static_cast<uint8_t>(paddingSize));
    }

    return paddedInput;
}

// Removes PKCS#7 padding from the input vector
vector<uint8_t> removePadding(const vector<uint8_t>& input) {
    if (input.empty()) {
        throw std::invalid_argument("Input is empty");
    }

    // Get the last byte which indicates the number of padding bytes
    uint8_t paddingSize = input.back();

    // Check for valid padding size
    if (paddingSize < 1 || paddingSize > BLOCK_SIZE) {
        throw std::invalid_argument("Invalid padding size");
    }

    // Check if the padding bytes match the padding size
    for (size_t i = 1; i <= paddingSize; ++i) {
        if (input[input.size() - i] != paddingSize) {
            throw std::invalid_argument("Invalid padding");
        }
    }

    // Remove padding
    return vector<uint8_t>(input.begin(), input.end() - paddingSize);
}

vector<uint8_t> hexStringToBytes(const string& hex) {
    vector<uint8_t> bytes;
    for (size_t i = 0; i < hex.length(); i += 2) {
        string byteString = hex.substr(i, 2);
        uint8_t byte = static_cast<uint8_t>(stoul(byteString, nullptr, 16));
        bytes.push_back(byte);
    }
    return bytes;
}

string bytesToHexString(const vector<uint8_t>& bytes) {
    stringstream ss;
    for (const auto& byte : bytes) {
        ss << hex << setw(2) << setfill('0') << static_cast<int>(byte);
    }
    return ss.str();
}

void RotWord(unsigned char* _w, unsigned char _d = 1, bool _right = 0) {
    //row is shifted _d times to the right if(_right) else left
    for (int r = 0; r < _d; r++)
    {
        unsigned char temp = _w[!_right ? 0 : 3];

        _w[!_right ? 0 : 3] = _w[!_right ? 1 : 2];
        _w[!_right ? 1 : 2] = _w[!_right ? 2 : 1];
        _w[!_right ? 2 : 1] = _w[!_right ? 3 : 0];
        _w[!_right ? 3 : 0] = temp;
    }
};

unsigned char SubByte(unsigned char _b)
{
    return sbox[_b];
}

void SubWord(unsigned char* _w) {
    for (int b = 0; b < 4; b++)
    {
        _w[b] = SubByte(_w[b]);
    }
}

// Key Expansion
void KeyExpansion(unsigned char* key, unsigned char* expandedKey) {
    const int Nk = 4; // Number of 32-bit words in the key (128 bits / 32 bits)
    const int Nb = 4; // Number of columns (32-bit words) in the state
    const int Nr = 10; // Number of rounds for AES-128

    for (int i = 0; i < Nk; i++) {
        expandedKey[i * 4]     = key[i * 4];
        expandedKey[i * 4 + 1] = key[i * 4 + 1];
        expandedKey[i * 4 + 2] = key[i * 4 + 2];
        expandedKey[i * 4 + 3] = key[i * 4 + 3];
    }

    unsigned char temp[4];
    // Generate round keys
    for (int i = Nk; i < Nb * (Nr + 1); i++) {
        temp[0] = expandedKey[(i - 1) * 4 + 0];
        temp[1] = expandedKey[(i - 1) * 4 + 1];
        temp[2] = expandedKey[(i - 1) * 4 + 2];
        temp[3] = expandedKey[(i - 1) * 4 + 3];

        if (i % Nk == 0) {
            // Rotate
            RotWord(temp);
            // Apply S-box
            SubWord(temp);

            // XOR with Rcon
            temp[0] ^= Rcon[i / Nk];
        } 

        expandedKey[i * 4 + 0] = expandedKey[(i - Nk) * 4 + 0] ^ temp[0];
        expandedKey[i * 4 + 1] = expandedKey[(i - Nk) * 4 + 1] ^ temp[1];
        expandedKey[i * 4 + 2] = expandedKey[(i - Nk) * 4 + 2] ^ temp[2];
        expandedKey[i * 4 + 3] = expandedKey[(i - Nk) * 4 + 3] ^ temp[3];
    }

    //cout << "DEBUGGING CHECK: PRINT EXPANDED KEY >>" << endl << expandedKey << endl << endl;
}

// Add Round Key
void AddRoundKey(unsigned char* state, unsigned char* roundKey) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

// Inverse Sub Bytes
void InvSubBytes(unsigned char* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = inv_sbox[state[i]];
    }
}

// Inverse Shift Rows
void InvShiftRows(unsigned char* state) {
    unsigned char tempState[16];
    unsigned char tempWord[4];

    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            tempWord[j] = state[(j * 4) + i];
        }

        RotWord(tempWord, i, 1);


        for (int j = 0; j < 4; j++)
        {
            tempState[(j * 4) + i] = tempWord[j];
        }
    }

    memcpy(state, tempState, 16);
}

uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1)
            p ^= a;
        bool hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set)
            a ^= 0x1B; // x^8 + x^4 + x^3 + x + 1
        b >>= 1;
    }
    return p;
}

// Inverse Mix Columns
void InvMixColumns(unsigned char* state) {
    for (int i = 0; i < 4; i++) {
        unsigned char a = state[i * 4 + 0];
        unsigned char b = state[i * 4 + 1];
        unsigned char c = state[i * 4 + 2];
        unsigned char d = state[i * 4 + 3];

        state[i * 4 + 0] = gmul(a, 14) ^ gmul(b, 11) ^ gmul(c, 13) ^ gmul(d, 9);
        state[i * 4 + 1] = gmul(a, 9) ^ gmul(b, 14) ^ gmul(c, 11) ^ gmul(d, 13);
        state[i * 4 + 2] = gmul(a, 13) ^ gmul(b, 9) ^ gmul(c, 14) ^ gmul(d, 11);
        state[i * 4 + 3] = gmul(a, 11) ^ gmul(b, 13) ^ gmul(c, 9) ^ gmul(d, 14);
    }
}

void decryptBlock(unsigned char* input, unsigned char* output, unsigned char* expandedKey) {
    const int Nr = 10; // Number of rounds for AES-128
    unsigned char state[16];
    memcpy(state, input, 16);

    // AddRoundKey (the last round key is added first)
    AddRoundKey(state, expandedKey + 16 * Nr);

    for (int round = Nr - 1; round > 0; round--) {
        InvShiftRows(state); // Inverse Shift Rows
        InvSubBytes(state);   // Inverse Sub Bytes
        AddRoundKey(state, expandedKey + 16 * round); // Add Round Key
        InvMixColumns(state); // Inverse Mix Columns
    }

    // Final round (no InvMixColumns)
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, expandedKey); // Add initial round key

    memcpy(output, state, 16);
}

vector<uint8_t> decryptHexString(const string& hexData, const string& _key) {
    if (_key.length() != 16) {
        throw invalid_argument("input key must be 16 bytes, current length: " + _key.length());
    }
    if (hexData.length() < (2 * BLOCK_SIZE)) {
        throw invalid_argument("input must be a multiple of 16 bytes, current length: " + (hexData.length() / 2));
    }

    // Convert hex string to bytes
    vector<uint8_t> encryptedData = hexStringToBytes(hexData);

    // Prepare expanded key
    unsigned char expandedKey[176]; // 16 bytes * (10 rounds + 1)
    unsigned char* key = new unsigned char[16];
    copy(_key.begin(), _key.end(), key);
    KeyExpansion(key, expandedKey);

    // Prepare the decrypted data buffer
    vector<uint8_t> decryptedData(encryptedData.size());

    for (size_t i = 0; i < encryptedData.size(); i += BLOCK_SIZE) {
        unsigned char block[16];
        size_t blockSize = min(size_t(16), encryptedData.size() - i);
        memcpy(block, &encryptedData[i], blockSize);

        unsigned char decryptedBlock[16];
        decryptBlock(block, decryptedBlock, expandedKey);

        memcpy(&decryptedData[i], decryptedBlock, blockSize);
    }

    return decryptedData;
}

vector<uint8_t> decryptB64String(const string& B64Data, const string& key) {
    return decryptHexString(b64_str_to_hex_str(B64Data), key);
}